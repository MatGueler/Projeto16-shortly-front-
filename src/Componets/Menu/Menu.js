...
function Menu() {
	...
	function encodeUrl(event) {
		...
		const promise = axios.post(
			`${process.env.REACT_APP_BACK_END_URL}/urls/shorten`,
			body,
			config
		);
	...
	function ShowMyUrls({ item }) {
		...
		function deleteUrl() {
			const promise = axios.delete(
				`${process.env.REACT_APP_BACK_END_URL}/urls/${item.id}`,
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
		...
		<Container>
			...
			<Main>
				...
				<UserUrls>
					{infos.length === 0
						? ''
						: infos.shortenedUrls.map((item, index) => {
								return <ShowMyUrls key={index} item={item} />;
						  })}
				</UserUrls>
			</Main>
		</Container>
